...
class Orders extends CI_Controller
{
    ...
    public function search()
    {
        ...
        $config['total_rows'] = $this->order->count_all_products();
        $config['per_page'] = 16;
        $config['uri_segment'] = 4;
        $choice = $config['total_rows'] / $config['per_page'];
        $config['num_links'] = floor($choice);
        ...
        $products['orders'] = $this->orders->search($query, $config['per_page'], $page);
        $products['pagination'] = $this->pagination->create_links();
        $products['count'] = $this->orders->count_search($query);

        $this->load->view('header', $params);
        $this->load->view('orders/search', $products);
        $this->load->view('footer');
    }
}
